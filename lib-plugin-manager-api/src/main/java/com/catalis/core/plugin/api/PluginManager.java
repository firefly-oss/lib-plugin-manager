...
public interface PluginManager {
     ...
     * @return a Mono that emits the plugin descriptor when the plugin has been installed
     */
    Mono<PluginDescriptor> installPluginFromGit(URI repositoryUri, String branch);

    /**
     * Installs a plugin from a Git repository using the default branch.
     *
     * @param repositoryUri the URI of the Git repository
     * @return a Mono that emits the plugin descriptor when the plugin has been installed
     */
    Mono<PluginDescriptor> installPluginFromGit(URI repositoryUri);

    /**
     * Scans the classpath for plugins annotated with @Plugin and installs them.
     *
     * @param basePackage the base package to scan (optional)
     * @return a Flux that emits the plugin descriptors for all installed plugins
     */
    Flux<PluginDescriptor> installPluginsFromClasspath(String basePackage);

    /**
     * Scans the entire classpath for plugins annotated with @Plugin and installs them.
     *
     * @return a Flux that emits the plugin descriptors for all installed plugins
     */
    Flux<PluginDescriptor> installPluginsFromClasspath();
